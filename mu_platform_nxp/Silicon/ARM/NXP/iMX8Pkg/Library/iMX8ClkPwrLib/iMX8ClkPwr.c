...
EFI_STATUS
IMXSetAudioPllClockRate (UINT32 TargetClockRate)
{
  ...
  // 1 + INT_DIV + (FRAC_DIV / 2^24) = PLL output freq * REFCLK_DIV * OUTDIV / ( 8 * Reference Freq ) >= 2
  {
    ...
    UINT32 intDiv = (((UINT64)TargetClockRate * refclkdiv * outdiv) / denom) - 1;
    ...
    pllAudioCfg1Reg.INT_DIV_CTL = intDiv;
  ...
  )
{
  ...
  status = IMXSetAudioPllClockRate (targetFreq);
  if (EFI_ERROR (status)) {
    return status;
  }
  ...
  ccmTargetRootAudioMclkReg.PRE_PODF = (preDivSelect - 1);
  ccmTargetRootAudioMclkReg.POST_PODF = (postDivSelect - 1);
