 ...
 * Definitions
 *****************************************************************************/
#define FSL_FEATURE_MU_NO_MUR 1U
#define FSL_FEATURE_MU_NO_PM 1U
#define FSL_FEATURE_MU_NO_HR 1U
#define FSL_FEATURE_MU_NO_RSTH 1U
#define FSL_FEATURE_MU_NO_CLKE 1U
#define FSL_FEATURE_MU_NO_NMI 1U
#define FSL_SDK_DISABLE_DRIVER_CLOCK_CONTROL 1U
/* Compatibility Macros */
#ifndef MU_CR_NMI_MASK
#define MU_CR_NMI_MASK 0U
#endif
#define MU_CR_Fn_MASK MU_CR_ABFn_MASK
#define MU_CR_Fn(X) MU_CR_ABFn(X)
...
static inline uint32_t MU_ReceiveMsgNonBlocking(MU_Type *base, uint32_t regIndex)
{
    ...
    return MmioRead32((UINTN)&base->RR[regIndex]);
}
...
{
    return (MmioRead32((UINTN)&base->SR) & MU_SR_Fn_MASK) >> MU_SR_Fn_SHIFT;
}
 ...
 */
static inline uint32_t MU_GetAcceptance(MU_Type *base)
{
    return MmioRead32((UINTN)&base->CR) & MU_CR_GIRn_MASK;
}
...
{
    return (MmioRead32((UINTN)&base->SR) & (MU_SR_TEn_MASK | MU_SR_RFn_MASK | MU_SR_GIPn_MASK | MU_SR_EP_MASK | MU_SR_FUP_MASK
#if (defined(FSL_FEATURE_MU_HAS_SR_RS) && FSL_FEATURE_MU_HAS_SR_RS)
                        | MU_SR_RS_MASK
#endif
#if (defined(FSL_FEATURE_MU_HAS_RESET_ASSERT_INT) && FSL_FEATURE_MU_HAS_RESET_ASSERT_INT)
...
{
    uint32_t irqMask = MmioRead32((UINTN)&base->CR) & (MU_CR_GIRn_MASK | MU_CR_TIEn_MASK | MU_CR_RIEn_MASK);
    return (MmioRead32((UINTN)&base->SR) & irqMask);
}
...
static inline void MU_ClearStatusFlags(MU_Type *base, uint32_t mask)
{
    ...
    MmioWrite32((UINTN)&base->SR, (mask & regMask));
}
 ...
 *
 * @code
 * if (0 != MU_TriggerInterrupts(base, kMU_GenInt0InterruptTrigger | kMU_GenInt2InterruptTrigger))
 * {
...
{
    MmioWrite32((UINTN)&base->SR, MU_SR_NMIC_MASK);
}
...
static inline void MU_HoldCoreBReset(MU_Type *base)
{
    ...
    MmioWrite32((UINTN)&base->CCR, MmioRead32((UINTN)&base->CCR) | MU_CCR_RSTH_MASK);
...
static inline void MU_ResetBothSides(MU_Type *base)
{
    ...
    while (0U != (MmioRead32((UINTN)&base->SR) & MU_SR_RS_MASK))
    {
    }
...
static inline void MU_MaskHardwareReset(MU_Type *base, bool mask)
{
    ...
    if (mask)
    {
        MmioWrite32((UINTN)&base->CCR, MmioRead32((UINTN)&base->CCR) | MU_CCR_HRM_MASK);
    }
    else
    {
        MmioWrite32((UINTN)&base->CCR, MmioRead32((UINTN)&base->CCR) & ~MU_CCR_HRM_MASK);
    ...
    if (mask)
    {
        MmioWrite32((UINTN)&base->CR, MmioRead32((UINTN)&base->CCR) | MU_CR_HRM_MASK);
    }
    else
    {
        MmioWrite32((UINTN)&base->CR, MmioRead32((UINTN)&base->CCR) & ~MU_CR_HRM_MASK);
...
static inline void MU_SetClockOnOtherCoreEnable(MU_Type *base, bool enable)
{
    ...
    if (enable)
    {
        MmioWrite32((UINTN)&base->CCR, MmioRead32((UINTN)&base->CCR) | MU_CCR_CLKE_MASK);
    }
    else
    {
        MmioWrite32((UINTN)&base->CCR, MmioRead32((UINTN)&base->CCR) & ~MU_CCR_CLKE_MASK);
...
{
    uint32_t ret = (MmioRead32((UINTN)&base->SR) & MU_SR_PM_MASK) >> MU_SR_PM_SHIFT;
