...
Device (PCI0)
{
  ...
  // PCIE_PL memory map
  OperationRegion (PLGC, SystemMemory, 0x5F010700, 0x220)
  Field (PLGC, DWordAcc, NoLock, Preserve) {
    Offset (0x2C),  // Skip to register 0x5F01072C
    DBG1, 32,       // PCIE_PL_DEBUG1
  }

  // PHYx1 memory map
  OperationRegion (PHYX, SystemMemory, 0x5F140000, 0x4000)
  Field (PHYX, DWordAcc, NoLock, Preserve) {
    Offset (0x0C),  // Skip to register 0x5F14000C
    pSTS, 32,       // PHYx1_STTS0
  }
  ...
  Method (_STA, 0x0, Serialized) {
    Name (LNK, 0x0);
    Name (STS0, 0x0);
    Store (pSTS, STS0);  // read PCIEPHY_RCR register

    // Check if the PCIe core is out of reset
    If (LNotEqual (And (STS0, 0x00080000), 0x00000000)) {
      Return (0x0)
    }
  ...
  Name (_PRT, Package() {
    ...
    // line.
    Package() {0x0000FFFF, 0, 0, 137}, // INTA
    Package() {0x0000FFFF, 1, 0, 138}, // INTB
    Package() {0x0000FFFF, 2, 0, 139}, // INTC
    Package() {0x0000FFFF, 3, 0, 140}, // INTD
  })
