...
Device (PCI0)
{
  ...
  // PCIE_PL memory map
  OperationRegion (PLGC, SystemMemory, 0x3380C700, 0x220)
  Field (PLGC, DWordAcc, NoLock, Preserve) {
    Offset (0x2C),  // Skip to register 0x3380C72C
    DBG1, 32,       // PCIE_PL_DEBUG1
  }

  // SRC memory map
  OperationRegion (SCR, SystemMemory, 0x30390000, 0x4000)
  Field (SCR, DWordAcc, NoLock, Preserve) {
    Offset (0x2C),  // Skip to register 0x3039002C
    pRCR, 32,       // PCIEPHY_RCR
  }
  ...
  Method (_STA, 0x0, Serialized) {
    Name (LNK, 0x0);
    Name (RCR, 0x0);
    Store (pRCR, RCR);  // read PCIEPHY_RCR register

    // Check if the PCIe PHY reference clock is enabled
    If (LNotEqual (And (RCR, 0x00000006), 0x00000000)) {
      Return (0x0)
    }
  ...
  Name (_CRS, ResourceTemplate () {
    WordBusNumber (
      ...
      7,                // AddressMaximum - Maximum Bus Number
      0,                // AddressTranslation - Set to 0
      8)                // RangeLength - Number of Busses
#else
    ...
    // PCI memory space
    Memory32Fixed (ReadWrite, FixedPcdGet32 (PcdPcie1DeviceMemoryBase), FixedPcdGet32 (PcdPcie1DeviceMemorySize), )
  })
  ...
  Name (_PRT, Package() {
    ...
    // line.
    Package() {0x0000FFFF, 0, 0, 157}, // INTA
    Package() {0x0000FFFF, 1, 0, 156}, // INTB
    Package() {0x0000FFFF, 2, 0, 155}, // INTC
    Package() {0x0000FFFF, 3, 0, 154}, // INTD/MSI
  })
...
Device (PCI1)
{
  ...
  // PCIE_PL memory map
  OperationRegion (PLGC, SystemMemory, 0x33C0C700, 0x220)
  Field (PLGC, DWordAcc, NoLock, Preserve) {
    Offset (0x2C),  // Skip to register 0x33C0C72C
    DBG1, 32,       // PCIE_PL_DEBUG1
  }
  ...
  OperationRegion (SCR, SystemMemory, 0x30390000, 0x4000)
  Field (SCR, DWordAcc, NoLock, Preserve) {
    Offset (0x48),  // Skip to register 0x30390048
    pRCR, 32,       // PCIEPHY_RCR
  }
  ...
  Name (_CRS, ResourceTemplate () {
    ...
    // PCI memory space
    Memory32Fixed (ReadWrite, FixedPcdGet32 (PcdPcie2DeviceMemoryBase), FixedPcdGet32 (PcdPcie2DeviceMemorySize), )
  })
  ...
  Name (_PRT, Package() {
    ...
    // is 0 and the fourth param is the SPI number of the interrupt
    // line.
    Package() {0x0000FFFF, 0, 0, 109}, // INTA
    Package() {0x0000FFFF, 1, 0, 108}, // INTB
    Package() {0x0000FFFF, 2, 0, 107}, // INTC
    Package() {0x0000FFFF, 3, 0, 106}, // INTD/MSI
  })
    ...
    If (LEqual (Arg0, ToUUID ("E5C937D0-3553-4d7a-9117-EA4D19C3434D"))) {
      ...
      case (7) {
          ...
          if (LEqual (Arg1, 2)) {
            ...
            Return ( Package (2) {
              1,            // Instance of the enumeration
              "PCIe Slot 2" // String name which matches the label on the chassis
            })
          }
        }
      }
