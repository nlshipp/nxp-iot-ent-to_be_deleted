  ...
  )
{
  ...
  if (HostInst->ExtCsdCardType & (MmcExtCsdCardTypeDdr1v8 | MmcExtCsdCardTypeDdr1v2)) {
    LOG_TRACE("Enabling DDR mode");
    HostInst->DevFlagDdr = TRUE;
  ...
  )
{
  ...
  HostExt = HostInst->HostExt;
  XfrInfo.BlockCount = MAX(1, BufferByteSize / SD_BLOCK_LENGTH_BYTES);
  XfrInfo.BlockSize = MIN(BufferByteSize, SD_BLOCK_LENGTH_BYTES);
  ...
  )
{
  ...
  if ((HostExt->DeviceType & CardFunctionSd) != 0) {
  ...
  if ((HostExt->DeviceType & (CardFunctionSdio |
                              CardFunctionComboSdSdio)) != 0) {
  ...
  if (HostInst->CardInfo.CardFunction != CardFunctionSd &&
    ...
    if ((HostExt->DeviceType & CardFunctionMmc) != 0) {
  ...
  )
{
  ...
  if (HostInst->CardInfo.CardFunction == CardFunctionSd) {
    if (HostInst->CardInfo.CurrentSpeedMode == CardSpeedModeNormalSpeed) {
      ...
      MaxClkFreqHz = SD_HIGH_SPEED_MODE_CLOCK_FREQ_HZ;
    }
  } else {
    if (HostInst->CardInfo.CurrentSpeedMode == CardSpeedModeNormalSpeed) {
      Status = CalculateCardMaxFreq (HostInst, &MaxClkFreqHz);
  ...
  HostExt->DevFlagDdr = HostInst->DevFlagDdr;
  ...
  )
{
  ...
  if (HostInst->DevFlagDdr && (BlockByteLength == 512)) {
	// CMD16 is illegal in DDR mode(s). Block length automatically default to 512 in DDR mode(s).
	return EFI_SUCCESS;
  }
  ...
  )
{
    ...
    CmdArg = (Mode << SD_SWITCH_MODE_OFFSET) | 0x00FFFFFF;
    CmdArg &= ~(SD_SWITCH_GRP_DFLT_FCN << (Group * SD_SWITCH_GRP_WIDTH));
    CmdArg |= (Value << (Group * SD_SWITCH_GRP_WIDTH));
  ...
  )
{
  ...
  UINT8       CmdStat[SD_SWITCH_STATUS_SIZE] = {0};

  ASSERT (HostInst);

  // Check if the SWITCH command is supported
  if ((HostInst->CardInfo.Registers.Sd.Csd.CCC & SD_CCC_SWITCH)) {
    // Get supported speed modes
    CmdArg = SetupSwitchCmdArg (SD_SWITCH_MODE_GET, SD_SWITCH_GRP_BUS_SPEED, SD_SWITCH_GRP_DFLT_FCN);
    Status = SdhcSendDataCommand (HostInst, &CmdSwitchSd, CmdArg, SD_SWITCH_STATUS_SIZE, HostInst->BlockBuffer);
    gBS->CopyMem (&CmdStat[0], HostInst->BlockBuffer, SD_SWITCH_STATUS_SIZE);

    if ((CmdStat[SD_SFS_GRP1_INF_L] & (1 << SD_SWITCH_HS))) {
      LOG_TRACE ("HS Card detected");
      // Select HS mode
      CmdArg = SetupSwitchCmdArg (SD_SWITCH_MODE_SET, SD_SWITCH_GRP_BUS_SPEED, SD_SWITCH_HS);
      Status = SdhcSendDataCommand (HostInst, &CmdSwitchSd, CmdArg, SD_SWITCH_STATUS_SIZE, HostInst->BlockBuffer);
      gBS->CopyMem (&CmdStat[0], HostInst->BlockBuffer, SD_SWITCH_STATUS_SIZE);
      if ((CmdStat[SD_SFS_GRP21_STAT] & SD_SFS_GRP1_MASK) != SD_SWITCH_HS) {
        Status = EFI_PROTOCOL_ERROR;
        LOG_ERROR ("HS mode selection error");
      } else {
        LOG_TRACE ("HS mode selected");
        HostInst->CardInfo.CurrentSpeedMode = CardSpeedModeHighSpeed;
      }
    }
  }
  ...
  )
{
  ...
  if (HostInst->DevFlagDdr) {
    SwitchCmdArg.Fields.Value |= MmcExtCsdBusWidthDdr;
  }
  ...
  )
{
  ...
  HostInst->ExtCsdCardType = ExtCsd->CardType;
