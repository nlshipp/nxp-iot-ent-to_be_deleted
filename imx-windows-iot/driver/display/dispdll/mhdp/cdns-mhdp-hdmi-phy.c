...
static void hdmi_arc_config(struct cdns_mhdp_device *mhdp)
{
	...
	prev_calib_code = (u16)cdns_phy_reg_read(mhdp, TX_DIG_CTRL_REG_2);
	txpu_calib_code = (u16)cdns_phy_reg_read(mhdp, CMN_TXPUCAL_CTRL);
	txpd_calib_code = (u16)cdns_phy_reg_read(mhdp, CMN_TXPDCAL_CTRL);
	txpu_adj_calib_code = (u16)cdns_phy_reg_read(mhdp, CMN_TXPU_ADJ_CTRL);
	txpd_adj_calib_code = (u16)cdns_phy_reg_read(mhdp, CMN_TXPD_ADJ_CTRL);

	new_calib_code = ((txpu_calib_code + txpd_calib_code) / 2)
		+ txpu_adj_calib_code + txpd_adj_calib_code;

	if (new_calib_code != prev_calib_code) {
		rdata = (u16)cdns_phy_reg_read(mhdp, TX_ANA_CTRL_REG_1);
		rdata &= 0xDFFF;
...
{
	const struct hdmi_ctrl *p_ctrl_table = NULL;
	const struct hdmi_pll_tuning *p_pll_table = NULL;
