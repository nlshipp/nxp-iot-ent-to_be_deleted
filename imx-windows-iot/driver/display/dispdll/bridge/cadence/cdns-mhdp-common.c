			    ...
			    u32 i_size, const u32 *d_mem, u32 d_size)
{
	...
	for (i = 0; i < (int)i_size; i += 4)
		cdns_mhdp_bus_write(*i_mem++, mhdp, ADDR_IMEM + i);

	for (i = 0; i < (int)d_size; i += 4)
			  ...
			  unsigned int block, size_t length)
{
	...
	for (i = 0; i < 4; i++) {
		...
		ret = cdns_mhdp_mailbox_read_receive(mhdp, edid, (u16)length);
		if (ret)
			continue;
...
int cdns_mhdp_config_video(struct cdns_mhdp_device *mhdp)
{
	...
	bit_per_pix = (video->color_fmt == YCBCR_4_2_2) ?
		      (u8)(video->color_depth * 2) : (u8)(video->color_depth * 3);
	...
	val = (u32)div_u64(mode->clock * (symbol + 1), 1000) + link_rate;
	val /= (mhdp->dp.num_lanes * link_rate);
	val = (u32)div_u64(8 * (symbol + 1), bit_per_pix) - val;
	val += 2;
