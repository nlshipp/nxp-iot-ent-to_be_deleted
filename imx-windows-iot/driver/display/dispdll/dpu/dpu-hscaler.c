...
int hscaler_pixengcfg_dynamic_src_sel(struct dpu_hscaler *hs, hs_src_sel_t src)
{
	...
	for (i = 0; i < ARRAY_SIZE(hs_id_array); i++)
		if ((int)hs_id_array[i] == hs->id)
			break;

	if (i == ARRAY_SIZE(hs_id_array)) {
		WARN_ON(1);
		return -EINVAL;
	}
...
int dpu_hs_deinit(struct dpu_soc *dpu, unsigned int id)
{
	struct dpu_hscaler *hs;
	int i;
	...
	if (hs->pec_base)
		devm_iounmap(dpu->dev, hs->pec_base, SZ_8);

	if (hs->base)
		devm_iounmap(dpu->dev, hs->base, SZ_1K);
