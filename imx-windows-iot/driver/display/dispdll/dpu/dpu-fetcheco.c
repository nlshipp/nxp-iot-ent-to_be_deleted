...
struct dpu_fetcheco *get_fe(struct dpu_fetchunit *fu)
{
	return container_of(fu, struct dpu_fetcheco, fu);
}
...
int dpu_fe_deinit(struct dpu_soc *dpu, unsigned int id)
{
	struct dpu_fetcheco *fe;
	struct dpu_fetchunit *fu;
	int i;
	...
	if (fu->base)
		devm_iounmap(dpu->dev, fu->base, SZ_128);
