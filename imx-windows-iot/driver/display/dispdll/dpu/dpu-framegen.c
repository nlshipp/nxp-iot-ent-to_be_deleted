...
static u32 framegen_get_fgchstat(struct dpu_framegen* fg)
{
	return dpu_fg_read(fg, FGCHSTAT);
}

int framegen_wait_done(struct dpu_framegen *fg, struct drm_display_mode *m)
{
	unsigned long long timeout, pending_framedur_jiffies;
	 ...
	 * So, four pending frames are enough for sure.
	 */
	pending_framedur_ns = (int)div_u64((u64) 4 * frame_size * 1000000, dotclock);
	...
	if (val & ENSTS) {
		dev_err(fg->dpu->dev, "failed to wait for FrameGen%d done, FGCHSTAT-0x%x\n",
			fg->id, framegen_get_fgchstat(fg));
		return -EINVAL;
	}
...
static void framegen_dump_clock(struct dpu_framegen* fg)
{
	dev_err(fg->dpu->dev, "FrameGen%d: clk-pll-rate-%d, clk-disp-rate-%d\n",
		fg->id, clk_get_rate(fg->clk_pll), clk_get_rate(fg->clk_disp));
}

int framegen_wait_for_frame_counter_moving(struct dpu_framegen *fg)
{
	u32 frame_index, line_index, last_frame_index;
	unsigned long long timeout = jiffies + msecs_to_jiffies(100);
	...
	if (last_frame_index == frame_index) {
		dev_err(fg->dpu->dev,
			"failed to wait for FrameGen%d frame counter moving (FGENSTS-0x%x)\n",
			fg->id, dpu_fg_read(fg, FGENSTS));
		framegen_dump_clock(fg);
		return -EINVAL;
	} else {
		dev_dbg(fg->dpu->dev,
...
void framegen_wait_for_secondary_syncup(struct dpu_framegen *fg)
{
	...
	if (syncup)
		...
		dev_err(fg->dpu->dev,
			"failed to wait for FrameGen%d secondary syncup: FGCHSTAT-0x%x\n",
			fg->id, framegen_get_fgchstat(fg));
}
...
bool framegen_is_master(struct dpu_framegen *fg)
{
	...
	return fg->id == (int)data->master_stream_id;
}
...
bool dpu_fg_is_enabled(struct dpu_soc *dpu, unsigned int id)
{
	struct dpu_framegen* fg;
	int i;
	...
	return dpu_fg_read(fg, FGENABLE) & FGEN;
}
...
int dpu_fg_deinit(struct dpu_soc *dpu, unsigned int id)
{
	struct dpu_framegen *fg = dpu->fg_priv[id];
	if (!fg)
		return -ENOMEM;

	if (fg->base)
		devm_iounmap(dpu->dev, fg->base, SZ_256);
