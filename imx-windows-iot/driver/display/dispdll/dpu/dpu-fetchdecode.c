...
struct dpu_fetchdecode *get_fd(struct dpu_fetchunit *fu)
{
	return container_of(fu, struct dpu_fetchdecode, fu);
}
...
int dpu_fd_deinit(struct dpu_soc *dpu, unsigned int id)
{
	struct dpu_fetchdecode *fd;
	struct dpu_fetchunit *fu;
	...
	if (fu->pec_base)
		devm_iounmap(dpu->dev, fu->pec_base, SZ_16);

	if (fu->base)
		devm_iounmap(dpu->dev, fu->base, SZ_1K);
