...
struct dpu_fetchlayer *get_fl(struct dpu_fetchunit *fu)
{
	return container_of(fu, struct dpu_fetchlayer, fu);
}
...
int dpu_fl_deinit(struct dpu_soc* dpu, unsigned int id)
{
	struct dpu_fetchlayer *fl;
	struct dpu_fetchunit *fu;
	...
	if (fu->base)
		devm_iounmap(dpu->dev, fu->base, SZ_512);
