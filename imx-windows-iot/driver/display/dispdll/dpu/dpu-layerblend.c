		...
		unsigned long pec_base, unsigned long base)
{
	...
	* display may continue to work with basic display driver).
	*
	* int ret = layerblend_pixengcfg_dynamic_prim_sel(lb, LB_PRIM_SEL__DISABLE);
	* if (ret < 0)
	*	return ret;
	*/
...
int dpu_lb_deinit(struct dpu_soc *dpu, unsigned int id)
{
	struct dpu_layerblend *lb;
	...
	if (lb->pec_base)
		devm_iounmap(dpu->dev, lb->pec_base, SZ_16);

	if (lb->base)
		devm_iounmap(dpu->dev, lb->base, SZ_32);
