...
int vscaler_pixengcfg_dynamic_src_sel(struct dpu_vscaler *vs, vs_src_sel_t src)
{
	...
	for (i = 0; i < ARRAY_SIZE(vs_id_array); i++)
		if ((int)vs_id_array[i] == vs->id)
			break;

	if (i == ARRAY_SIZE(vs_id_array)) {
		WARN_ON(1);
		return -EINVAL;
	}
...
int dpu_vs_deinit(struct dpu_soc *dpu, unsigned int id)
{
	struct dpu_vscaler *vs;
	int i;
	...
	if (vs->pec_base)
		devm_iounmap(dpu->dev, vs->pec_base, SZ_8);

	if (vs->base)
		devm_iounmap(dpu->dev, vs->base, SZ_1K);
