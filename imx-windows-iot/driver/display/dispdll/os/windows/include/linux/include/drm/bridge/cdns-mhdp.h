...
#define cdns_mhdp_plat_call_void(mhdp, operation)			\
	(!(mhdp) ? (void)0 : (((mhdp)->plat_data && (mhdp)->plat_data->operation) ?	\
	 (mhdp)->plat_data->operation(mhdp) : (void)0))
...
struct cdns_mhdp_device {
	char __iomem		*regs_base;
	char __iomem		*regs_sec;
