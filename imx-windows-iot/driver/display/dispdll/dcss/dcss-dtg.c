			       ...
			       struct platform_device *pdev)
{
	...
	dcss_writel(LINE0_IRQ | LINE1_IRQ,
...
int dcss_dtg_init(struct dcss_dev *dcss, unsigned long dtg_base)
{
	...
	/*
	* Keep DTG running in case it has been left running by the bootloader so
	* that controller can be disabled gracefully during full modeset.
	*/
	if (dcss_readl(dtg->base_reg + DCSS_DTG_TC_CONTROL_STATUS) & DTG_START) {
...
void dcss_dtg_sync_set(struct dcss_dtg *dtg, struct videomode *vm)
{
	...
	dtg_lrc_x = (u16)(vm->hfront_porch + vm->hback_porch + vm->hsync_len +
		    vm->hactive - 1);
	dtg_lrc_y = (u16)(vm->vfront_porch + vm->vback_porch + vm->vsync_len +
		    vm->vactive - 1);
	dis_ulc_x = (u16)(vm->hsync_len + vm->hback_porch - 1);
	dis_ulc_y = (u16)(vm->vsync_len + vm->vfront_porch + vm->vback_porch - 1);
	dis_lrc_x = (u16)(vm->hsync_len + vm->hback_porch + vm->hactive - 1);
	dis_lrc_y = (u16)(vm->vsync_len + vm->vfront_porch + vm->vback_porch +
		    vm->vactive - 1);

	/* dcss pixel clock is generated by HDMI PHY
	* so its configuration is handled by mhdp phy driver.
	*/
			    ...
			    int px, int py, int pw, int ph)
{
	...
	p_ulc_x = (u16)(dtg->dis_ulc_x + px);
	p_ulc_y = (u16)(dtg->dis_ulc_y + py);
	p_lrc_x = (u16)(p_ulc_x + pw);
	p_lrc_y = (u16)(p_ulc_y + ph);
