	...
	if (dcss_dpr_ch_init_all(dpr, dpr_base)) {
		...
		for (i = 0; i < 3; i++) {
			if (dpr->ch[i].base_reg)
				iounmap(dpr->ch[i].base_reg, SZ_4K);
		}
...
void dcss_dpr_exit(struct dcss_dpr *dpr, bool stop)
{
	int ch_no;
...
void dcss_dpr_addr_set_no_ctxld(struct dcss_dpr *dpr, int ch_num,
	u32 base_addr)
{
	struct dcss_dpr_ch *ch = &dpr->ch[ch_num];

	if (!dcss_dtrc_ch_running(dpr->dtrc, ch_num))
		dcss_writel(base_addr, ch->base_reg + DCSS_DPR_FRAME_1P_BASE_ADDR);
}

void dcss_dpr_tile_set_no_ctxld(struct dcss_dpr *dpr,
	int ch_num, uint64_t modifier)
{
	...
	dcss_writel(ch->mode_ctrl, ch->base_reg + DCSS_DPR_MODE_CTRL0);
}

void dcss_dpr_tile_set_ctxld(struct dcss_dpr *dpr,
	int ch_num, uint64_t modifier)
{
	struct dcss_dpr_ch *ch = &dpr->ch[ch_num];
...
u32 dcss_get_dpr_base_addr(struct dcss_dpr *dpr, int ch_num)
{
	struct dcss_dpr_ch *ch = &dpr->ch[ch_num];
	return dcss_readl(ch->base_reg + DCSS_DPR_FRAME_1P_BASE_ADDR);
}
