...
{
	int curr_ctx = ctxld->current_ctx;
	struct dcss_ctxld_item *ctx[3];
	ctx[CTX_DB] = ctxld->db[curr_ctx];
	ctx[CTX_SB_HP] = ctxld->sb_hp[curr_ctx];
	ctx[CTX_SB_LP] = ctxld->sb_lp[curr_ctx];
...
{
	int ret = 0;
	unsigned long long timeout = jiffies + msecs_to_jiffies(500);
