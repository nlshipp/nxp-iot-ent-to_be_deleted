...
void dcss_ss_exit(struct dcss_ss *ss, bool stop)
{
	/* stop SS */
	if (stop)
		      ...
		      bool phsync, bool pvsync)
{
	...
	lrc_x = (u16)(vm->hfront_porch + vm->hback_porch + vm->hsync_len +
		vm->hactive - 1);
	lrc_y = (u16)(vm->vfront_porch + vm->vback_porch + vm->vsync_len +
		vm->vactive - 1);

	dcss_ss_write(ss, (lrc_y << LRC_Y_POS) | lrc_x, DCSS_SS_DISPLAY);

	hsync_start = (u16)(vm->hfront_porch + vm->hback_porch + vm->hsync_len +
		      vm->hactive - 1);
	hsync_end = (u16)(vm->hsync_len - 1);

	dcss_ss_write(ss, (phsync ? SYNC_POL : 0) |
		      ((u32)hsync_end << SYNC_END_POS) | hsync_start,
		      DCSS_SS_HSYNC);

	vsync_start = (u16)(vm->vfront_porch - 1);
	vsync_end = (u16)(vm->vfront_porch + vm->vsync_len - 1);

	dcss_ss_write(ss, (pvsync ? SYNC_POL : 0) |
		      ((u32)vsync_end << SYNC_END_POS) | vsync_start,
		      DCSS_SS_VSYNC);

	de_ulc_x = (u16)(vm->hsync_len + vm->hback_porch - 1);
	de_ulc_y = (u16)(vm->vsync_len + vm->vfront_porch + vm->vback_porch);

	dcss_ss_write(ss, SYNC_POL | ((u32)de_ulc_y << ULC_Y_POS) | de_ulc_x,
		      DCSS_SS_DE_ULC);

	de_lrc_x = (u16)(vm->hsync_len + vm->hback_porch + vm->hactive - 1);
	de_lrc_y = (u16)(vm->vsync_len + vm->vfront_porch + vm->vback_porch +
		   vm->vactive - 1);
