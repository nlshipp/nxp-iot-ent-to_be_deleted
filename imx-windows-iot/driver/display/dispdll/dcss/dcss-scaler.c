	...
	if (dcss_scaler_ch_init_all(scaler, scaler_base)) {
		...
		for (i = 0; i < 3; i++) {
			if (scaler->ch[i].base_reg)
				iounmap(scaler->ch[i].base_reg, SZ_4K);
		}
				      ...
				      enum chroma_location src_chroma_loc)
{
	...
	downscale_factor = (u8)dcss_scaler_factors[ch->ch_num].downscale;
	upscale_factor = (u8)dcss_scaler_factors[ch->ch_num].upscale;

	/* return if WR_SCL/RD_SRC is needed to scale */
	return l_vinc > (u32)downscale_fp(downscale_factor, 13)  ||
	       l_vinc < (u32)upscale_fp(upscale_factor, 13)	    ||
	       l_hinc >(u32)downscale_fp(downscale_factor, 13)  ||
	       l_hinc < (u32)upscale_fp(upscale_factor, 13);
}
