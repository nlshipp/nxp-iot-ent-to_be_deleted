...
CCapturePin (
    ...
    ,m_PixelFmt (kVIDEO_PixelFormatNone)
...

{
    ...
    if (!NT_SUCCESS (Status)) {
        ...
        m_PixelFmt = (video_pixel_format_t)((reinterpret_cast <PKS_DATAFORMAT_VIDEOINFOHEADER> (m_Pin->ConnectionFormat))->DataFormat.SubFormat.Data1);
    }
...

{
    ...
    if (!Leading) {
        ...
        while (NT_SUCCESS (Status) && Leading) {
            ...
            FrameInfo_t frameInfo {(SPContext->BufferVirtual), Leading->OffsetOut.Count, 0, FALSE};

            if (ClonePointer->StreamHeader->Size >= sizeof(KSSTREAM_HEADER)+sizeof(KS_FRAME_INFO)) {

                PKS_FRAME_INFO FrameInfo = reinterpret_cast <PKS_FRAME_INFO> (ClonePointer->StreamHeader+1);

                if (FrameInfo->lSurfacePitch != 0) {
                    frameInfo.m_Stride = FrameInfo->lSurfacePitch;
                    frameInfo.m_StrideValid = true;
                    if (frameInfo.m_Stride < 0) {
                        frameInfo.m_Stride = -frameInfo.m_Stride;
                    }
                }
            }
            if (NT_SUCCESS(Status)) {

                auto *csi = m_Device->m_Csi.getTarget();

                if (csi == NULL) {
                    Status = STATUS_INVALID_DEVICE_STATE;
                } else {
                    PIRP LowIrp = IoBuildDeviceIoControlRequest(IOCTL_CSI_DRIVER_GET_FRAME, csi->m_TargetDevicePtr, &frameInfo, sizeof(frameInfo), frameInfo.m_Virtual, frameInfo.m_ByteCount, FALSE, &csi->m_Event, &csi->m_IoStatus);
            ...
            if (MappingsUsed > 0) {
                Status = KsStreamPointerAdvanceOffsets (Leading, 0, frameInfo.m_ByteCount, FALSE);
            } else {
    ...
    if (MappingsUsed > 0) { // Othrwise STATUS_PENDING
        CompleteMappings(MappingsUsed);
    } else {
        // ASSERT(m_PendIo == FALSE);
...
Routine Description:
    ...
    mappings have completed.  Let the buffers go if possible.IoBuildDeviceIoControlRequest
    We're called at DPC.
...
FormatYUY2_Capture720p = {
    ...
    {   
        ...
        0,                                      // Reserved
        STATICGUIDOF (KSDATAFORMAT_TYPE_VIDEO), // aka. MEDIATYPE_Video
        kVIDEO_PixelFormatYUYV, 0x0000, 0x0010, 0x80, 0x00, // MEDIASUBTYPE_YUY2
    ...
    //
    {
        ...
        0,              // ShrinkTapsX 
        0,              // ShrinkTapsY 
        333667,         // MinFrameInterval, 100 nS units - 29 fps
        640000000,      // MaxFrameInterval, 100 nS units
...
FormatUYVY_Capture720p = {
    ...
    //
    {
        ...
        0,                                      // Reserved
        STATICGUIDOF(KSDATAFORMAT_TYPE_VIDEO), // aka. MEDIATYPE_Video
        kVIDEO_PixelFormatUYVY, 0x0000, 0x0010, 0x80, 0x00, // MEDIASUBTYPE_UYVY
        0x00, 0xaa, 0x00, 0x38, 0x9b, 0x71,
        STATICGUIDOF(KSDATAFORMAT_SPECIFIER_VIDEOINFO) // aka. FORMAT_VideoInfo
    },
...
FormatNV12_Capture720p = {
    ...
    //
    {
        sizeof(KS_DATARANGE_VIDEO),            // FormatSize
        0,                                      // Flags
        // (ULONG)(DMAX_X * DMAX_Y),                    // SampleSize
        (ULONG)((DMAX_X * DMAX_Y * 12 ) / 8),                    // SampleSize
        0,                                      // Reserved
        STATICGUIDOF(KSDATAFORMAT_TYPE_VIDEO), // aka. MEDIATYPE_Video
        kVIDEO_PixelFormatNV12, 0x0000, 0x0010, 0x80, 0x00, // MEDIASUBTYPE_UYVY
        0x00, 0xaa, 0x00, 0x38, 0x9b, 0x71,
        STATICGUIDOF(KSDATAFORMAT_SPECIFIER_VIDEOINFO) // aka. FORMAT_VideoInfo
    },
    ...
    //
    {
        ...
        (DWORD)(DMAX_X * DMAX_Y * 1.5 + 0.5),              // DWORD biSizeImage;
        0,                                  // LONG  biXPelsPerMeter;
        0,                                  // LONG  biYPelsPerMeter;
        0,                                  // DWORD biClrUsed;
        0                                   // DWORD biClrImportant;
...
NTSTATUS CCapturePin::Close(_In_ PKSPIN Pin,_In_ PIRP Irp)
{
    return STATUS_SUCCESS;
}

void CCapturePin::Reset(_In_ PKSPIN Pin)
{}
...
//
// DECLARE_SIMPLE_FRAMING_EX(FramingExName, MemoryType, Flags, Frames, Alignment, MinFrameSize, MaxFrameSize) \
//
...
CapturePinCsiDataRanges[] = {
    (PKSDATARANGE)&FormatYUY2_Capture720p
};
...
CapturePinIsiDataRanges[] = {
    (PKSDATARANGE)&FormatYUY2_Capture720p,
    (PKSDATARANGE)&FormatNV12_Capture720p
};

SIZE_T GetCsiCapturePinRangesCount() {
    return SIZEOF_ARRAY(CapturePinCsiDataRanges);
}

SIZE_T GetIsiCapturePinRangesCount() {
    return SIZEOF_ARRAY(CapturePinIsiDataRanges);
}
