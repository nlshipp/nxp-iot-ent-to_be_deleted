...

{
    ...
    if (!Leading) {
        ...
        while (NT_SUCCESS (Status) && Leading) {
            ...
            FrameInfo_t frameInfo {(SPContext->BufferVirtual), Leading->OffsetOut.Count,
                0,
                FALSE,
            };
            if(ClonePointer->StreamHeader->Size >= sizeof(KSSTREAM_HEADER)+sizeof(KS_FRAME_INFO))
            {
                ...
                {
                    frameInfo.Stride = FrameInfo->lSurfacePitch;
                    frameInfo.StrideValid = true;
                    if(frameInfo.Stride < 0){
                        frameInfo.Stride = -frameInfo.Stride;
                    }
                }
            }
            if (NT_SUCCESS(Status)) {
				...
				if (csi == NULL) {
					...
					PIRP LowIrp = IoBuildDeviceIoControlRequest(IOCTL_CSI_DRIVER_GET_FRAME, csi->m_TargetDevicePtr, &frameInfo, sizeof(frameInfo), frameInfo.Virtual, frameInfo.ByteCount, FALSE, &csi->m_Event, &csi->m_IoStatus);
					if (LowIrp) {
            ...
            if (MappingsUsed > 0) {
                Status = KsStreamPointerAdvanceOffsets (Leading, 0, frameInfo.ByteCount, FALSE);
            } else {
...
CapturePinDataRanges [CAPTURE_PIN_DATA_RANGE_COUNT] = {
    (PKSDATARANGE) &FormatUYVY_Capture720p,
