...
// get connection parameters from resourse hub/rhproxy and validate them
    ...
    status = FindControllerClockSettings(pDevice, i2cDescriptorPtr->ConnectionSpeed);

    if (status != STATUS_SUCCESS) {
        TraceEvents(TRACE_LEVEL_ERROR, TRACE_DEVICE,
            "I2C connection speed %lu is not complied with all requirements.",
            i2cDescriptorPtr->ConnectionSpeed);
    ...
    )
{
    ...
    pRequest->BytesAlreadyRequested = 0;
