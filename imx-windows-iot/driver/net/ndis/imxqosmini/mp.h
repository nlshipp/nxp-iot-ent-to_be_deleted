...
typedef enum _MP_MII_PHY_INTERFACE_TYPE {
    RGMII,
    RMII,
    MII
} MP_MII_PHY_INTERFACE_TYPE, * PMP_MII_PHY_INTERFACE_TYPE;
...
typedef struct _MP_ADAPTER {
    ...
    BOOLEAN                 EnetStarted;              // Set to TRUE(data path is running) by EnetStart() and to FALSE in EnetQos_Stop() method.
    ...
    struct _MP_MII_CFG {
        UINT8                          PhyAddr;             // PHY device address on MDIO bus.
        MP_MII_PHY_INTERFACE_TYPE      MiiInterfaceType;    // PHY device interface type (MII/RMMI/RGMII)
    } MiiCfg;
    ULONG                   CcmBase;                               // Physical address of the CCM module
    ULONG                   CcmSize;                               // Size of the CCM module
    ...
    PMP_TX_PAYLOAD_BD       Tx_EnetSwExtBDT;                       // Address of table containing Sw related data for each Enet BD
