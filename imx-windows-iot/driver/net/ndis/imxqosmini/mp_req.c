    ...
    *pCounter = 0;
    switch (Oid) {
        case OID_GEN_XMIT_OK:
            *pCounter = ENETRegBase->MAC_TX_PACKET_COUNT_GOOD.R;
            break;
        ...
        case OID_GEN_STATISTICS:
            ...
            //<SC> pAdapter->StatisticsAcc.ifInDiscards += (ULONG64)ENETRegBase->RMON_R_CRC_ALIGN + ENETRegBase->RMON_R_UNDERSIZE + ENETRegBase->RMON_R_OVERSIZE + ENETRegBase->IEEE_R_MACERR;
            ...
            OID_GEN_RCV_ERROR */
            //<SC> pAdapter->StatisticsAcc.ifInErrors += (ULONG64)ENETRegBase->RMON_R_CRC_ALIGN + ENETRegBase->RMON_R_UNDERSIZE + ENETRegBase->RMON_R_OVERSIZE;
            ...
            pAdapter->StatisticsAcc.ifHCInOctets = ENETRegBase->MAC_RX_OCTET_COUNT_GOOD_BAD.R;
            ...
            pAdapter->StatisticsAcc.ifHCInUcastPkts = ENETRegBase->MAC_RX_PACKETS_COUNT_GOOD_BAD.R - ENETRegBase->MAC_RX_MULTICAST_PACKETS_GOOD.R - ENETRegBase->MAC_RX_BROADCAST_PACKETS_GOOD.R;
            ...
            pAdapter->StatisticsAcc.ifHCInMulticastPkts = ENETRegBase->MAC_RX_MULTICAST_PACKETS_GOOD.R;
            ...
            pAdapter->StatisticsAcc.ifHCInBroadcastPkts = ENETRegBase->MAC_RX_BROADCAST_PACKETS_GOOD.R;
            ...
            pAdapter->StatisticsAcc.ifHCOutOctets = ENETRegBase->MAC_TX_OCTET_COUNT_GOOD_BAD.R;
            ...
            pAdapter->StatisticsAcc.ifHCOutUcastPkts = ENETRegBase->MAC_TX_UNICAST_PACKETS_GOOD_BAD.R;
            ...
            pAdapter->StatisticsAcc.ifHCOutMulticastPkts = ENETRegBase->MAC_TX_MULTICAST_PACKETS_GOOD.R;
            ...
            pAdapter->StatisticsAcc.ifHCOutBroadcastPkts = ENETRegBase->MAC_TX_BROADCAST_PACKETS_GOOD.R;
            ...
            OID_GEN_XMIT_ERROR */
            //<SC> pAdapter->StatisticsAcc.ifOutErrors += (ULONG64)ENETRegBase->IEEE_T_EXCOL + ENETRegBase->IEEE_T_LCOL;
            ...
            //<SC> pAdapter->StatisticsAcc.ifOutDiscards += ENETRegBase->IEEE_T_EXCOL;
...
NDIS_STATUS NICSetPacketFilter(PMP_ADAPTER pAdapter,  ULONG PacketFilter)
{
    ...
    if (Filter & NDIS_PACKET_TYPE_PROMISCUOUS )  {
        ...
        pAdapter->ENETRegBase->MAC_PACKET_FILTER.R = IMX_ENET_QOS_MAC_PACKET_FILTER_PR_MASK;
    } else {
    ...
    if (Filter & NDIS_PACKET_TYPE_MULTICAST ) {
        pAdapter->ENETRegBase->MAC_PACKET_FILTER.R = IMX_ENET_QOS_MAC_PACKET_FILTER_PM_MASK;
    } else {
        pAdapter->ENETRegBase->MAC_PACKET_FILTER.R &= ~IMX_ENET_QOS_MAC_PACKET_FILTER_PM_MASK;
    }
    ...
    do {
        ...
        if (NewPowerState == NdisDeviceStateD0) {
            ...
            if (pAdapter->EnetStarted) {                                                        // Is ENET running?
                ...
                NdisReleaseSpinLock(&pAdapter->Dev_SpinLock);
                EnetQos_Stop(pAdapter, NDIS_STATUS_LOW_POWER_STATE);                            // Stop the adapter and remember new NDIS status.
...
NDIS_STATUS MpSetInformation(NDIS_HANDLE MiniportAdapterContext, PNDIS_OID_REQUEST NdisRequest)
{
    ...
    switch(Oid) {
        ...
        case OID_802_3_MULTICAST_LIST:
            ...
            NdisMoveMemory(pAdapter->MCList, InformationBuffer, (size_t)MCAddressCount * ETH_LENGTH_OF_ADDRESS);   // Save the MC list
            BytesRead = InformationBufferLength;
            NdisAcquireSpinLock(&pAdapter->Dev_SpinLock);
            ClearAllMultiCast(pAdapter);                  // Clear all MC addresses
